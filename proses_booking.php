...
if (empty($tanggal)){
  ...
  if(mysql_num_rows($cekuser)>0){
    ...
    while ($c){
    if( $c['jam_main'] <= $jam_main && $c['selesai'] >= $jam_main){
       echo"<script language=\"javascript\">alert(\"MAAF !!! Jadwal Sudah DI Booking\"); document.location.href='t_booking.php';</script>";
    } 
    else if ($c['jam_main'] < $selesai && $c['selesai'] >= $selesai){
       echo"<script language=\"javascript\">alert(\"MAAF !!! Jadwal Sudah DI Booking\"); document.location.href='t_booking.php';</script>";
    }  
    else if ($c['jam_main'] >= $jam_main && $c['selesai'] <= $selesai){
       echo"<script language=\"javascript\">alert(\"MAAF !!! Jadwal Sudah DI Booking\"); document.location.href='t_booking.php';</script>";
    } 
    else if  ($c['selesai'] == $selesai){
       echo"<script language=\"javascript\">alert(\"MAAF !!! Jadwal Sudah DI Booking\"); document.location.href='t_booking.php';</script>";
    } 
    else if ($c['jam_main'] == $jam_main){ 
      echo"<script language=\"javascript\">alert(\"MAAF !!! Jadwal Sudah DI Booking\"); document.location.href='t_booking.php';</script>";     
  }
   ...
   $sql = 'SELECT * FROM booking';
    ...
    $ambildata1 = mysql_query( $sql1, $koneksi);
      ...
      mysql_query($query);
        echo"<script language=\"javascript\">alert(\"Berhasil Tanggal sama !!! Jadwal Sudah Berhasil Di Booking\"); document.location.href='t_booking.php';</script>";
      } 
}else{
      $koneksi = mysql_connect("localhost","root","");
    ...
    $ambildata1 = mysql_query( $sql1, $koneksi);
      ...
      $query = mysql_query("INSERT INTO booking (tanggal,jam_main,selesai,lapangan,Namatim,harga,Status) values ('$tanggal','$jam_main','$selesai','$lapangan','$Namatim','$hasil1','Booked')");
      mysql_query($query);
        echo"<script language=\"javascript\">alert(\"Berhasil Tanggal Beda !!! Jadwal Sudah Berhasil Di Booking\"); document.location.href='t_booking.php';</script>";
    }
  }
?>